...
- 分卷答案按“题型分区＋题号”匹配，支持名词解释、简答、计算、论述等题型重新编号。
- 原 PDF 连续阅读，跨页显示上下页及各自选框；兼容 Tk 9 触控板滚动，同页切换截图复用页面图像。
- 题干、选项、答案与解析统一紧凑截图，排除重复页眉、页脚和题型标题。
- 上传编辑器未就绪时有限重试，分组查找支持重新加载；失败窗口可直接重试原批次。
- 新版界面与任务进度窗口，保留处理结果；导出完成后可直接进入上传。
